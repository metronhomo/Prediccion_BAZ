...
plot_distribucion_modelo <- function(df, var) {
  text = paste0(
    "gg_temp <-", get_name(df), "%>% 
    mutate(q05 = quantile(",var, ", 0.05, na.rm=TRUE),
    ...
    geom_vline(aes(xintercept = c(q05, q25, q50, q75, q90, q95)),
    size = 0.5, color = 'red') +
    facet_wrap(~respuesta) +
    ylab('Number of customers')
    min = min(unique(gg_temp$data$q05))
    max = max(unique(gg_temp$data$q95))
    gg_temp + 
    xlim(min,  max) +
    geom_histogram(binwidth = abs(max - min)/30)
    ")
